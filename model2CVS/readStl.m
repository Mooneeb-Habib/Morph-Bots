...
fclose(fid);
%If the term 'facet normal' does not exist within strFile, then use Binary style format.
...
for idx = 1:numTri
    ...
    %ditch normal - Normal Vector (which this code is not using i.e. 'ditch')

    fread(fid, 3, 'single');
    
    %Finding out the 3 vertices of a triangle
    ...
    %ditch attibute
    %Attribute Byte Count - Set to zero and acts as a spacer b/w two triangles
    fread(fid, 1, 'uint16');
end
